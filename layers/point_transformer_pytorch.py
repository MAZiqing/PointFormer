...
class PointTransformerLayer(nn.Module):
    ...
    def forward(self, x, pos, mask=None):
        ...
        rel_pos_emb = self.pos_mlp(rel_pos) # [B, N, 3] -> [B, N, hidden]
        ...
        if exists(num_neighbors) and num_neighbors < n:
            ...
            v_select = batched_index_select(v, indices, dim=2)
        ...
        # aggregate
        agg = einsum('b i j d, b i j d -> b i d', attn, v_select)
        return agg


if __name__ == '__main__':
    model = PointTransformerLayer(dim=32, pos_mlp_hidden_dim=8, attn_mlp_hidden_mult=4, num_neighbors=5)
